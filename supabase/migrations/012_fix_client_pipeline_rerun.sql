-- 011 was accidentally run twice. Its second run failed partway through
-- (CREATE TYPE client_application_status already exists), which could leave
-- the database in one of several states depending on whether that batch was
-- rolled back. This migration is fully idempotent — safe to run no matter
-- what state 011 left things in — and produces the same end result as a
-- clean single run of 011.
DROP TABLE IF EXISTS clients CASCADE;
DROP TYPE IF EXISTS client_application_status CASCADE;
